...
const UpdateGroupChatModal = ({ fetchAgain, setFetchAgain }) => {
  ...
  const onSearchUserHandler = async (query) => {
    ...
    try {
      ...
      let link = `http://localhost:8050/api/user?search=${searchUser}`;
  ...
  const updateGroupName = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(
        "http://localhost:8050/api/chats/rename",
        {
  ...
  const removeUser = async (user1) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        "http://localhost:8050/api/chats/removeuser",
        {
          groupId: selectedChat._id,
          userId: user1._id,
        },
  ...
  const addNewUser = async (newUser) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        "http://localhost:8050/api/chats/adduser",
        {
          groupId: selectedChat._id,
          userId: newUser._id,
        },
